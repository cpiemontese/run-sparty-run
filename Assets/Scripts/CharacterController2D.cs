...
public class CharacterController2D : MonoBehaviour
{
    ...
    public bool doubleJumpIsUnlocked = false;


    // player health
    public int currentPlayerHealth = 10;
    public int startingPlayerHealth = 10;
    ...
    // timer used to ignore collisions with platforms
    float _ignoreCollisionMaxTime = 0.25f;
    float _ignoreCollisionsTimer = 0f;
    ...
    void Update()
    {
        ...
        // If can double jump AND jump button pressed, then allow the player to double jump
        else if (doubleJumpIsUnlocked && _canDoubleJump && CrossPlatformInputManager.GetButtonDown("Jump"))
        {
            DoJump();
            _canDoubleJump = false;
        }
        else if (_isGrounded && verticalInput < 0f && Mathf.Approximately(_ignoreCollisionsTimer, 0f))
        {
            _ignoreCollisionsTimer = _ignoreCollisionMaxTime;
        }

        if (_ignoreCollisionsTimer > 0.0f)
        {
            _ignoreCollisionsTimer -= Time.deltaTime;
        }
        else
        {
            _ignoreCollisionsTimer = 0.0f;
        }
        ...
        Physics2D.IgnoreLayerCollision(_playerLayer, _platformLayer, _vy > 0.0f || _ignoreCollisionsTimer > 0.0f);
    }
